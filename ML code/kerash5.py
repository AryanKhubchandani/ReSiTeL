...
class Classifier:
    ...
    def getPrediction(self, img, draw= True, pos=(50, 50), scale=2, color = (0,255,0)):
        ...
        prediction_probabilit = prediction[0, prediction.argmax()]
        if draw and self.labels_path and prediction_probabilit<0.7:
            cv2.putText(img, str(self.list_labels[indexVal]),
                        pos, cv2.FONT_HERSHEY_COMPLEX, scale, color, 2)

        return  prediction_probabilit



def main():
    cap = cv2.VideoCapture(0)
    maskClassifier = Classifier('model.tflite', 'labels.txt')
    while True:
        _, img = cap.read()
        prediction_probability = maskClassifier.getPrediction(img)
